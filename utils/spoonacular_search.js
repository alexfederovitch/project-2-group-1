...
let searchList = [];
    getSearch = function () {
        const search = {
            url: `https://spoonacular-recipe-food-nutrition-v1.p.mashape.com/recipes/recipes/search?
            number=10&query=${query}&limitLicense=false`,
            headers: {
        ...
        request.get(search, function (error, response, body) {
            let searchList = JSON.parse(body);
    ...
    const details = {
        url: `https://spoonacular-recipe-food-nutrition-v1.p.mashape.com/recipes/informationBulk?` +
            ...
            `ids=${mealsFound.join("%2C")}&includeNutrition=true`,
        headers: {
            "X-Mashape-Key": process.env.MASH_APIKEY,
            "X-Mashape-Host": "spoonacular-recipe-food-nutrition-v1.p.mashape.com",
        }
    ...
    request.get(details, function (error, response, body) {
        ...
        for (let i = 0; i < detailsList.length; i++) {
            ...
            mealsDetails[i] = Object.assign(searchList[i], {
