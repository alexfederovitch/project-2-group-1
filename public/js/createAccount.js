...
$("#button").on("click", function (event) {
  ...
  let pw2 = $("#confirm-password").val().trim();
  let currentWeight = $("#weight").val().trim();
  let goal = $("input[name='goalOption']:checked").val();

  console.log(firstName,lastName, email, username, pw1, pw2, currentWeight, goal);
...
console.log(newUserVals.password,newUserVals.password2,newUserVals.password === newUserVals.password2)
  if (newUserVals.password === newUserVals.password2) {
    firebase.auth().createUserWithEmailAndPassword(newUserVals.email, newUserVals.password)
      .then(function (user) {
        ...
        $.ajax("/api/newUser", {
          ...
          if (result.isworking) {
            ...
            localStorage.setItem("currenWeight", result.dbUsers.currentWeight);
